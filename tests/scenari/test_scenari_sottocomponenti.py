...
@pytest.mark.parametrize(
    "caso",
    CASI_ADDIZIONALE_REGIONALE,
    ids=[c.descrizione for c in CASI_ADDIZIONALE_REGIONALE] or None,
)
...
@pytest.mark.parametrize(
    "caso",
    CASI_ADDIZIONALE_COMUNALE,
    ids=[c.descrizione for c in CASI_ADDIZIONALE_COMUNALE] or None,
)
...
def test_detrazione_lavoro_dipendente(caso):
    risultato = REGIME_2026_MILANO.detrazione_lavoro_dipendente(caso.imponibile, caso.giorni_lavorati, caso.tempo_determinato)
...
@pytest.mark.parametrize(
    "caso",
    CASI_TRATTAMENTO_INTEGRATIVO,
    ids=[c.descrizione for c in CASI_TRATTAMENTO_INTEGRATIVO] or None,
)
def test_trattamento_integrativo(caso):
    risultato = REGIME_2026_MILANO.trattamento_integrativo(caso.imponibile, caso.irpef_lorda, caso.detrazione, caso.giorni_lavorati)
    assert risultato == caso.atteso
